...
class ProductProduct(models.Model):
    ...
    def _compute_available_quantities_dict(self):
        ...
        all_products_with_bom = self.env["product.product"]
        products_to_process = product_with_bom
        processed_products = self.env["product.product"]
        
        while products_to_process:
            current_batch = products_to_process
            products_to_process = self.env["product.product"]
            ...
            for product in current_batch:
                ...
                if bom_id:
                    for line in bom_id.bom_line_ids:
                        ...
                        if component and (component.variant_bom_ids or component.bom_ids):
                            if component not in processed_products:
                                products_to_process |= component

        exploded_boms = all_products_with_bom._explode_boms()
        ...
        sorted_products = sorted(
            all_products_with_bom,
            key=lambda p: self._get_bom_depth(p, exploded_boms),
            reverse=True
        )
        ...
        for product in sorted_products:
            ...
            if not component_needs:
                ...
                for component, need in component_needs.items():
                    base_qty = component_qties[component.id][stock_available_mrp_based_on]
                    component_potential = res.get(component.id, {}).get("potential_qty", 0.0)
                    total_available = base_qty + component_potential
                    available_quantities.append(total_available / need)
                
                components_potential_qty = min(available_quantities)
    ...
    def _get_components_needs(self, exploded_components):
        ...
        for bom_line, bom_qty in exploded_components:
            ...
            if not component:  # Skip BOM lines without product
                continue
            needs += Counter({component: bom_qty})

        return needs

    def _get_bom_depth(self, product, exploded_boms):
        max_depth = 0
        if product.id not in exploded_boms:
            return 0
        for bom_line, _ in exploded_boms[product.id]:
            component = bom_line.product_id
            if component and component.id in exploded_boms:
                depth = 1 + self._get_bom_depth(component, exploded_boms)
                if depth > max_depth:
                    max_depth = depth
        return max_depth
